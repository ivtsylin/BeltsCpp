...
struct AddressHasher {
  // реализуйте структуру
    ...
    size_t operator() (const Address& item) const {
        size_t h_city = shash(item.city);
        size_t h_street = shash(item.street);
        size_t h_building = ihash(item.building);
        constexpr size_t p = 31;
        
        return h_city + h_street * p + h_building * p * p;
    }
};
...
struct PersonHasher {
  // реализуйте структуру
    ...
    size_t operator() (const Person& item) const {
        size_t h_name = shash(item.name);
        size_t h_height = ihash(item.height);
        size_t h_weight = dhash(item.weight);
        size_t h_address = ahash(item.address);
        constexpr size_t p = 31;
        
        return h_name + h_height * p + h_weight * p * p + h_address * p * p * p;
    }
};
