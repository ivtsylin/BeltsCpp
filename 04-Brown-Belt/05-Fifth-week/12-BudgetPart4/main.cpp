...
array<double, VERTEX_COUNT> tree_spend_values, tree_spend_add, tree_values, tree_add, tree_factor;
...
void PushSpend(size_t v, size_t l, size_t r) {
  for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
    if (w < VERTEX_COUNT) {
      tree_spend_add[w] += tree_spend_add[v];
      tree_spend_values[w] += tree_spend_add[v] * (r - l) / 2;
    }
  }
  tree_spend_add[v] = 0;
}
...
double ComputeSumSpend(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return 0;
  }
  PushSpend(v, l, r);
  if (ql <= l && r <= qr) {
    return tree_spend_values[v];
  }
  return ComputeSumSpend(v * 2, l, (l + r) / 2, ql, qr)
      + ComputeSumSpend(v * 2 + 1, (l + r) / 2, r, ql, qr);
}
...
void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr, double rate) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return;
  }
  Push(v, l, r);
  ...
  Multiply(v * 2, l, (l + r) / 2, ql, qr, rate);
  Multiply(v * 2 + 1, (l + r) / 2, r, ql, qr, rate);
  tree_values[v] =
      (v * 2 < VERTEX_COUNT ? tree_values[v * 2] : 0)
      + (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1] : 0);
}
...
int main() {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    if (query_type == "ComputeIncome") {
      cout << ComputeSum(1, 0, DAY_COUNT_P2, idx_from, idx_to) - ComputeSumSpend(1, 0, DAY_COUNT_P2, idx_from, idx_to) << endl;
    } else if (query_type == "PayTax") {
      double value;
      cin >> value;
      Multiply(1, 0, DAY_COUNT_P2, idx_from, idx_to, 1.0 - value / 100);
