...
public:
    ...
    explicit ConcurrentMap(size_t bucket_count) : data(bucket_count), muts(bucket_count) {};

    WriteAccess operator[](const K& key) {
        size_t ind = hasher(key) % data.size();
        return {lock_guard(muts[ind]), data[ind][key]};
    }
    
    ReadAccess At(const K& key) const {
        size_t ind = hasher(key) % data.size();
        return {lock_guard(muts[ind]), data[ind].at(key)};
    }
    
    bool Has(const K& key) const {
        size_t ind = hasher(key) % data.size();
        auto lg = lock_guard(muts[ind]);
        const MapType& current = data[ind];
        return current.find(key) != current.end();
    }
