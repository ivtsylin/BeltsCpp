...
void TestPack() {
    {
        ...
        {
            auto result = RequestStat(bus_catalog).get_bus_stats("256");
        ...
        {
            auto result = RequestStat(bus_catalog).get_bus_stats("750");
            ASSERT(result.bus_stats.has_value());
        ...
        {
            auto result = RequestStat(bus_catalog).get_bus_stats("751");
            ASSERT(!result.bus_stats.has_value());
        }
    }
    {
        ...
        {
            auto result = RequestStat(bus_catalog).get_stop_stats("Samara");
            ASSERT(!result.bus_name_set.has_value());
        }
        {
            auto result = RequestStat(bus_catalog).get_stop_stats("Prazhskaya");
            ASSERT(result.bus_name_set.has_value());
            ASSERT(result.bus_name_set.value().begin() == result.bus_name_set.value().end());
        }
        {
            auto result = RequestStat(bus_catalog).get_stop_stats("Biryulyovo Zapadnoye");
            ASSERT(result.bus_name_set.has_value());
