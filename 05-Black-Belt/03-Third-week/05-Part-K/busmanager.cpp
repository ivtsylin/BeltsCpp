...
void BusManager::prepeare_for_map() {
    if (is_prepeared)
        return;
    ...
    for (const auto& [_, bus_route] : buses.get_data()) {
        const auto& route = bus_route.get_data();
        if (route.size() > 1) {
            for (int i = 0; i < route.size() - 1; ++i) {
                adjacent_stops[route[i]].insert(route[i + 1]);
                adjacent_stops[route[i + 1]].insert(route[i]);
            }
        }
    }
}
...
void BusManager::read_bus_catalog(const Json::Document& doc) {
    ...
    for (const auto& lays : rsettings.at("layers").AsArray()) {
        render_setting.layers.push_back(lays.AsString());
    }
...
Json::Document BusManager::process_and_print_requests(const Json::Document& doc) {
    ...
    for (const auto& stat_req : stat_requests) {
        ...
        if (stat_req_map.at("type").AsString() == "Bus") {
            ...
            prepeare_for_map();
            stops.prepeare_for_render(render_setting.height, render_setting.width, render_setting.padding, adjacent_stops);
            
            int color_palette_size = render_setting.color_palette.size();
            int cur_color_num;
            
            for (const string& layer : render_setting.layers) {
                ...
                if (layer == "bus_lines") {
                    ...
                    for (const auto& [_, bus_route] : buses.get_data()) {
                        ...
                        for (const auto& stop_name : bus_route.get_data()) {
                            poly.AddPoint({
                                stops.get_data().find(stop_name)->second.get_normalized_coordinates().lon,
                                stops.get_data().find(stop_name)->second.get_normalized_coordinates().lat
                            });
                        }
                    ...
                    for (const auto& [bus_name, bus_route] : buses.get_data()) {
                        ...
                        Svg::Text()
                            .SetPoint({
                                stops.get_data().find(first_stop)->second.get_normalized_coordinates().lon,
                                stops.get_data().find(first_stop)->second.get_normalized_coordinates().lat
                            })
                        ...
                        if (const string& last_stop = bus_route.get_data()[bus_route.get_last_stop_index()];
                            ...
                            Svg::Text()
                                .SetPoint({
                                    stops.get_data().find(last_stop)->second.get_normalized_coordinates().lon,
                                    stops.get_data().find(last_stop)->second.get_normalized_coordinates().lat
                                })
                                .SetOffset(render_setting.bus_label_offset)
                                .SetFontSize(render_setting.bus_label_font_size)
                                .SetFontFamily("Verdana")
                    ...
                    for (const auto& [_, stop_info] : stops.get_data()) {
                        Svg::Circle()
                            .SetCenter({
                                stop_info.get_normalized_coordinates().lon,
                                stop_info.get_normalized_coordinates().lat
                            })
