...
// Класс-оболочка на двумя итераторами
...
// Функция, которая разбивает строку на две
// Возвращает второй аргумент в виде optional
...
// Функция, которая разбивает строку на две
// Возвращает второй аргумент в виде пустой строки
...
// Функция, которая разбивает строку на две
// Если второй подстроки нет, то считает, что вторая подстрока пустая
// Возвращает первую часть строки
// Подменяет входную строку на вторую подстроку
...
// Если строка оказалась длиннее, то выкидывает исключение
...
// Валидирует, что число принадлежит границам
...
// Класс Интервал
// Определяет принадлежность элемента интервалу
struct IndexSegment {
  size_t left;
  size_t right;
...
struct FinStreams {
    double in = 0.0;
    double out = 0.0;
    FinStreams operator+(const FinStreams& rhs) const{
        return {in + rhs.in, out + rhs.out};
    }
    FinStreams &operator+=(const FinStreams& rhs) {
        in += rhs.in;
        out += rhs.out;
        return *this;
    }
    FinStreams operator*(const double rhs) {
        in *= rhs;
        return *this;
    }
};
...
struct BulkTaxApplier {
  BulkTaxApplier(int tax_percentage = 0)
      : factor(1.0 - tax_percentage / 100.0){};
  double factor;

  double ComputeFactor() const { return factor; }
};

// Класс применения двух бюджетных записей в одно действие
...
public:
  ...
  FinStreams Collapse(FinStreams origin, IndexSegment segment) const {
      return {origin.in * tax_.ComputeFactor() + add_.delta * segment.length(),
      origin.out + spend_.delta * segment.length() };
  }
...
class BudgetManager : public SummingSegmentTree<FinStreams, BulkLinearUpdater> {
public:
...
struct ComputeIncomeRequest : ReadRequest<double> {
  ...
  double Process(const BudgetManager& manager) const override {
      FinStreams Result = manager.ComputeSum(MakeDateSegment(date_from, date_to));
...
struct PayTaxRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier{rate});
  }
